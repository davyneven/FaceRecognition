...
Recognition::Recognition(const char *facerecAlgorithm)
{
...
void Recognition::learnCollectedFaces(const vector<Mat> preprocessedFaces, const vector<int> faceLabels)
{
...
void Recognition::showTrainingDebugData(const int faceWidth, const int faceHeight)
{
...
Mat Recognition::reconstructFace(const Mat preprocessedFace)
{
...
bool Recognition::loadModel(const char *fileName) {
    Mat labels;
    try {
        model->load(fileName);
        labels = model->get<Mat>("labels");
...
void Recognition::safeModel(const char *fileName) {
    model->save(fileName);

}

int Recognition::predict(const Mat preprocessedFace) {
    return model->predict(preprocessedFace);
}
