...
const char *databaseFilePath = "/home/davy/RecognitionDatabase/";
...
enum MODES {MODE_STARTUP=0, MODE_DETECTION, MODE_COLLECT_FACES, MODE_TRAINING, MODE_RECOGNITION, MODE_DELETE_ALL,MODE_SAVE_ALL, MODE_LOAD, MODE_END, MODE_PROFILE_PIC};
const char* MODE_NAMES[] = {"Startup", "Detection", "Collect Faces", "Training", "Recognition", "Delete All","Save All", "Load", "ERROR!", "Taking profile pic"};
MODES m_mode = MODE_STARTUP;

int m_selectedPerson = -1;
int m_numPersons = 0;
vector<int> m_latestFaces;
vector<Mat> profile_pics;
...
void onMouse(int event, int x, int y, int, void*)
{
    ...
    if (isPointInRect(pt, m_rcBtnAdd)) {
        ...
        if ((m_numPersons == 0) || (m_latestFaces[m_numPersons-1] >= 0)) {
            // Add a new person, take profile pic
            m_numPersons++;
    ...
    else if (isPointInRect(pt, m_rcBtnSave)) {
        cout << "User clicked [Save] button." << endl;
        m_mode = MODE_SAVE_ALL;
    }
    else if(isPointInRect(pt, m_rcBtnLoad)) {
        cout << "User clicked [Load] button." << endl;
        m_mode = MODE_LOAD;
    }
    else {
...
void recognizeAndTrainUsingWebcam(VideoCapture &videoCapture)
{
    ...
    while (true) {
        ...
        cout << "Size: " << cameraFrame.size() << endl;
        ...
        if(m_mode == MODE_PROFILE_PIC)
        {
            Rect profileRect = Rect(faceRect.x - 50, faceRect.y - faceRect.height*0.5, faceRect.width*1.5, faceRect.height*2)& Rect(0,0,640,360);
            Mat profilePic;
            cameraFrame(profileRect).copyTo(profilePic);
            resize(profilePic, profilePic, Size(80,100));
            profile_pics.push_back(profilePic);
            imshow("test", profilePic);
            m_mode = MODE_COLLECT_FACES;
        }
            ...
            if (gotFaceAndEyes) {
                ...
                if (old_prepreprocessedFace.data) {
                    imageDiff = recognizer->getSimilarity(preprocessedFace, old_prepreprocessedFace);
                }
            ...
            if (m_numPersons < 1 || preprocessedFaces.size() <= 0 || preprocessedFaces.size() != faceLabels.size()) {
                cout << "num persons: " << m_numPersons << endl;
                cout << "preprocc size: " << preprocessedFaces.size() << endl;
                cout << "face labels size: " << faceLabels.size() << endl;
            ...
            if (haveEnoughData) {
                ...
                cout <<" size preprocessedFaces: " << preprocessedFaces.size() << endl;
                cout <<" size faceLabels: " << faceLabels.size() << endl;
                recognizer->learnCollectedFaces(preprocessedFaces, faceLabels);
                cout << "after training" << endl;
                // Show the internal face recognition data, to help debugging.
                if (m_debug)
                    recognizer->showTrainingDebugData(faceWidth, faceHeight);
        ...
        else if (m_mode == MODE_SAVE_ALL)
        {
            string path = format("%sdatabase.xml", databaseFilePath);
            FileStorage fs(path, FileStorage::WRITE);
            fs << "images" << "[";
            for(int i = 0; i< preprocessedFaces.size(); i++)
            {
                Mat img = preprocessedFaces[i];
                string fileName = format("%straining_images/%d.jpg",databaseFilePath, i);
                imwrite(fileName, img);
                fs << fileName;
            }
            fs << "]";
            fs << "profile_images" << "[";
            for(int i = 0; i< profile_pics.size(); i++)
            {
                Mat img = profile_pics[i];
                string fileName = format("%sprofile_images/%d.jpg",databaseFilePath, i);
                imwrite(fileName, img);
                fs << fileName;
            }
            fs << "]";
            ...
            fs << "]";
            fs.release();
            path = format("%sFaceModel.yml", databaseFilePath);
            recognizer->safeModel(path.data());
            m_mode = MODE_DETECTION;
        }
        else if(m_mode == MODE_LOAD)
        {
            //clear all previous images and labels in vector
            preprocessedFaces.clear();
            faceLabels.clear();
            //reading from xml
            string path = format("%sdatabase.xml", databaseFilePath);
            ...
            m_numPersons = (int)fs["numPersons"];
            if(m_numPersons == 0)
            {
                cerr << "not enough persons in xml file to train recognizer!" << endl;
                m_mode = MODE_DETECTION;
                continue;
            }
            // reading images
            ...
            for (; it != it_end; ++it)
            {
                ...
                profile_pics.push_back(img);
            }
            n = fs["labels"];
            if (n.type() != FileNode::EMPTY)
            {
                cerr << "labels is not a sequence! FAIL" << endl;
            }
            ...
            if (n.type() != FileNode::SEQ)
            {
                cerr << "faces is not a sequence! FAIL" << endl;
            }
            it = n.begin(); it_end = n.end();
            for (; it != it_end; ++it)
            {
                int face = (int)*it;
                m_latestFaces.push_back(face);
            }
            m_mode = MODE_TRAINING;
        }
        else {
        ...
        m_rcBtnSave = drawButton(displayedFrame, "Save", Point(m_rcBtnDebug.x, m_rcBtnDebug.y + m_rcBtnDebug.height), m_rcBtnAdd.width);
        m_rcBtnLoad = drawButton(displayedFrame, "Load", Point(m_rcBtnSave.x, m_rcBtnSave.y + m_rcBtnSave.height), m_rcBtnAdd.width);
