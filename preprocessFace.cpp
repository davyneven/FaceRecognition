...
void detectBothEyes(const Mat &face, dlib::shape_predictor &shapePredictor, Point &leftEye, Point &rightEye)
{
...
Mat getPreprocessedFace(Mat &srcImg, int desiredFaceWidth, dlib::frontal_face_detector &faceDetector, dlib::shape_predictor &shapePredictor, bool doLeftAndRightSeparately, Rect *storeFaceRect, Point *storeLeftEye, Point *storeRightEye)
{
    ...
    if(dets.size() > 0)
    {
    	faceRect.height = dets[0].height();
    	faceRect.width = dets[0].width();
    	faceRect.x = dets[0].left();
    	faceRect.y = dets[0].top();

    	if (faceRect.x < 0)
    		faceRect.x = 0;
		if (faceRect.y < 0)
			faceRect.y = 0;
		if (faceRect.x + faceRect.width > srcImg.cols)
			faceRect.x = srcImg.cols - faceRect.width;
		if (faceRect.y + faceRect.height > srcImg.rows)
			faceRect.y = srcImg.rows - faceRect.height;
    ...
    if (faceRect.width > 0) {
        ...
        detectBothEyes(faceImg, shapePredictor,leftEye, rightEye);
